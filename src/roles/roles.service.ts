...
export class RolesService {
    ...
    async deleteRole(userId: number, assocationId: number): Promise<boolean> {
        const result = await this.repository.delete({idUser: userId, idAssociation: assocationId});
        return result.affected > 0;
    }

    async update(idUser: number, idAssociation: number, newRole: string) : Promise<Role> {
