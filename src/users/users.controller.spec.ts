...
describe('UsersController', () => {
  ...
  describe('getAll', () => {
    it('should return an array of users', async () => {
      ...
      let out = await controller.getAll();
      console.log(out);
      expect(out).toBe(await expected);
    });
  });
  ...
  describe('getById', () => {
    it('should return a single user, with the provided id', async () => {
      ...
      jest.spyOn(service, 'getUser').mockImplementation((id) => {
        return Promise.resolve(expected[id]);
      });
      expect(await controller.getUser({ id: 0 })).toBe(await expected[0]);
    });
  });
});
